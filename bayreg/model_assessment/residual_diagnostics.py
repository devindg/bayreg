...
def get_projection_matrix_diagonal(predictors, prior_coeff_prec=None):
    ...
    if prior_coeff_prec is None:
        ...
        US = U @ S
        W = mat_inv(StS + Vt @ prior_coeff_prec @ Vt.T)
        for i in range(n):
...
def studentized_residuals(
    response, predictors, prior_coeff_prec=None, prior_coeff_mean=None
):
    y = np.array(response.copy())
    x = np.array(predictors.copy())
    ...
    if x.ndim not in (1, 2):
        raise ValueError(
            "The predictors array must have dimension 1 or 2. Dimension is 0."
        )
    elif x.ndim == 1:
        x = x.reshape(-1, 1)
    else:
    ...
    if n != x.shape[0]:
        raise ValueError(
            "The response and predictors arrays do not have the same number "
            "of observations."
        )

    if prior_coeff_prec is not None and prior_coeff_mean is None:
        raise ValueError(
            "prior_coeff_mean must be provided if prior_coeff_prec is provided."
        )

    if prior_coeff_prec is None and prior_coeff_mean is not None:
        raise ValueError(
            "prior_coeff_prec must be provided if prior_coeff_mean is provided."
        )
    ...
    ext_stud_resid = int_stud_resid * np.sqrt(
        (n - p - 1) / (n - p - int_stud_resid**2)
    )
